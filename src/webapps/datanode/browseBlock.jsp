<%@ page
  ...
  import="edu.indiana.d2i.algorithm.ProvenanceTracking"
%>
...
<%!
  ...
  public void generateFileChunks(JspWriter out, HttpServletRequest req) 
    ...
    out.print("<div style=\"width:100%;height: 400px;background-color: #98AFC7;border:1px solid gray;overflow: auto;\">");
    try {
    jspHelper.streamBlockInAscii(
            ...
            "/browseBlock.jsp?filename="+filename+"&backtrack=");
    } catch (Exception e){
        out.print(e);
    }
    //out.print("</textarea>");
    ...
    dfs.close();
  	...
  	// uri right now is just a key
	public void generateProvenance(JspWriter out, HttpServletRequest req, String uri)
		throws IOException {
		// get the job ID from the log file name, hard code!!!
		String jobID = null;
		String filename = req.getParameter("filename");
    	if (filename == null) {
      		out.print("Invalid input (filename absent)");
    	...
    	String parent = new File(filename).getParent();
    	String logPath = parent + "/" + "_logs/history"; // this is the default path
		DFSClient dfs = new DFSClient(jspHelper.nameNodeAddr, jspHelper.conf);
		if (!dfs.exists(logPath) || !dfs.isDirectory(logPath)) {
      		out.print("<h3>File or directory : " + logPath + " does not exist</h3>");
      		return;
    	}
    	FileStatus[] files = dfs.listPaths(logPath);
    	for (int i = 0; i < files.length; i++) {
    		String confname = files[i].getPath().toString();
    		if (confname.endsWith(".xml")) {
    			int begin = confname.indexOf("job", 0);
				int end = confname.indexOf("_conf", 0);
				jobID = confname.substring(begin, end);
				break;
    		}
    	}
    	if (jobID == null) {
    		out.print("<h3>Unable to get jobID</h3>");
    		return;
    	}
		
		// invoke the tracking algorithm
		ProvenanceTracking tracking = new ProvenanceTracking(jobID, uri, jspHelper.conf, out);
		tracking.backTrack();
		
		//out.println("<p>" + jobID + "</p>");
   		//out.println(uri);
  	}
...
<% 
	String uri = request.getParameter("backtrack");
    if (uri != null) {
   		generateProvenance(out, request, uri);
    }
	else {
