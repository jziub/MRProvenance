...
public class JspHelper {
	...
	public static final UnixUserGroupInformation webUGI = UnixUserGroupInformation
			.createImmutable(conf.getStrings(WEB_UGI_PROPERTY_NAME));

	public static final int defaultChunkSizeToView = conf.getInt(
			"dfs.default.chunk.view.size", 32 * 1024);
	...
	public DatanodeInfo bestNode(LocatedBlock blk) throws IOException {
		...
		while (s == null) {
			...
			InetSocketAddress targetAddr = NetUtils.createSocketAddr(chosenNode
					.getHost() + ":" + chosenNode.getInfoPort());
	...
	public void streamBlockInAscii(InetSocketAddress addr, long blockId,
			long genStamp, long blockSize, long offsetIntoBlock,
			long chunkSizeToView, JspWriter out, String backtrackURL)
			throws IOException {
		...
		DFSClient.BlockReader blockReader = DFSClient.BlockReader
				.newBlockReader(s, addr.toString() + ":" + blockId, blockId,
						genStamp, offsetIntoBlock, amtToRead,
		...
		/* print out the provenance links */
		StringBuilder output = new StringBuilder();
		String[] splits = new String(buf).split("\n");
		for (int i = 0; i < splits.length; i++) {
			// get the key out from the key value pair
			String[] strs = splits[i].split("\t");
			if (strs.length == 0) strs = splits[i].split(" ");
			String key = strs[0];

			output.append("<a href=\"" + backtrackURL + key + "\""); // /browseBlock.jsp?backtrack=key
			output.append("onclick=\"OpenPopup(this.href); return false\">");
	...
	public void addTableRow(JspWriter out, String[] columns, int row)
			throws IOException {
		out.print("<tr>");

		for (int i = 0; i < columns.length; i++) {
			if (row / 2 * 2 == row) {// even
				out.print("<td style=\"vertical-align: top;background-color:LightGrey;\"><B>"
						+ columns[i] + "</B><br></td>");
			} else {
				out.print("<td style=\"vertical-align: top;background-color:LightBlue;\"><B>"
						+ columns[i] + "</B><br></td>");
	...
	public String getInodeLimitText() {
		...
		String str = inodes + " files and directories, " + blocks
				+ " blocks = " + (inodes + blocks) + " total";
		if (maxobjects != 0) {
	...
	public void sortNodeList(ArrayList<DatanodeDescriptor> nodes, String field,
			String order) {

		class NodeComapare implements Comparator<DatanodeDescriptor> {
			static final int FIELD_NAME = 1, FIELD_LAST_CONTACT = 2,
					FIELD_BLOCKS = 3, FIELD_CAPACITY = 4, FIELD_USED = 5,
					FIELD_PERCENT_USED = 6, FIELD_NONDFS_USED = 7,
					FIELD_REMAINING = 8, FIELD_PERCENT_REMAINING = 9,
					SORT_ORDER_ASC = 1, SORT_ORDER_DSC = 2;
			...
			public int compare(DatanodeDescriptor d1, DatanodeDescriptor d2) {
				...
				case FIELD_PERCENT_USED:
					double ddbl = ((d1.getDfsUsedPercent()) - (d2
							.getDfsUsedPercent()));
					ret = (ddbl < 0) ? -1 : ((ddbl > 0) ? 1 : 0);
					break;
				case FIELD_PERCENT_REMAINING:
					ddbl = ((d1.getRemainingPercent()) - (d2
							.getRemainingPercent()));
					ret = (ddbl < 0) ? -1 : ((ddbl > 0) ? 1 : 0);
					break;
				case FIELD_BLOCKS:
	...
	public static void printPathWithLinks(String dir, JspWriter out,
			int namenodeInfoPort) throws IOException {
		try {
			...
			out.print("<a href=\"browseDirectory.jsp" + "?dir="
					+ Path.SEPARATOR + "&namenodeInfoPort=" + namenodeInfoPort
					+ "\">" + Path.SEPARATOR + "</a>");
			...
			for (int i = 0; i < parts.length - 1; i++) {
				if (!parts[i].equals("")) {
					...
					out.print("<a href=\"browseDirectory.jsp" + "?dir="
							+ tempPath.toString() + "&namenodeInfoPort="
							+ namenodeInfoPort);
	...
	public static void printGotoForm(JspWriter out, int namenodeInfoPort,
		...
		out.print("Goto : ");
		out.print("<input name=\"dir\" type=\"text\" width=\"50\" id\"dir\" value=\""
				+ file + "\">");
	...
	public static void createTitle(JspWriter out, HttpServletRequest req,
			String file) throws IOException {
		if (file == null)
			file = "";
