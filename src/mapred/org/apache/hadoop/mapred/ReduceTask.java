...
import edu.indiana.d2i.ReduceRecordWritterWrapper;
...
class ReduceTask extends Task {
	...
	public void run(JobConf job, final TaskUmbilicalProtocol umbilical)
		...
		if (useNewApi) {
			LOG.info("runNewReducer");
			runNewReducer(job, umbilical, reporter, rIter, comparator,
					keyClass, valueClass);
		} else {
			LOG.info("runOldReducer");
	...
	private <INKEY, INVALUE, OUTKEY, OUTVALUE> void runNewReducer(JobConf job,
		...
		org.apache.hadoop.mapreduce.Reducer<INKEY, INVALUE, OUTKEY, OUTVALUE> reducer = new ReducerWrapper<INKEY, INVALUE, OUTKEY, OUTVALUE>(
				(org.apache.hadoop.mapreduce.Reducer<INKEY, INVALUE, OUTKEY, OUTVALUE>) ReflectionUtils
						.newInstance(taskContext.getReducerClass(), job), taskContext);
		...
		// reduceOutputCounter);
		String outputFile = org.apache.hadoop.mapreduce.lib.output.FileOutputFormat.getOutputFileName();
		org.apache.hadoop.mapreduce.RecordWriter<OUTKEY, OUTVALUE> trackedRW = new ReduceRecordWritterWrapper<OUTKEY, OUTVALUE>(
				new NewTrackingRecordWriter<OUTKEY, OUTVALUE>(output,
						reduceOutputCounter), taskContext, outputFile);
		
//		Path path = FileOutputFormat.getOutputPath(job);
//		LOG.info("ReduceTask output path " + path.getName());
//		LOG.info("" + org.apache.hadoop.mapreduce.lib.output.FileOutputFormat.getOutputFileName());
		
		// change to wrapper class
		valueClass = (Class) MapOutputValueWrapper.class;
