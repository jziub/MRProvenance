...
import edu.indiana.d2i.util.ProvenanceInforGenerator;
...
class MapTask extends Task {
	...
	class SkippingRecordReader<K, V> extends TrackedRecordReader<K, V> {
		...
		SkippingRecordReader(RecordReader<K, V> raw,
			...
			this.skipRecCounter = reporter
					.getCounter(Counter.MAP_SKIPPED_RECORDS);
	...
	private <INKEY, INVALUE, OUTKEY, OUTVALUE> void runOldMapper(
			final JobConf job, final BytesWritable rawSplit,
	 ...
	 */
	private static class OldOutputCollector<K, V> implements
			OutputCollector<K, V> {
	...
	private class NewDirectOutputCollector<K, V> extends
		...
		NewDirectOutputCollector(
				org.apache.hadoop.mapreduce.JobContext jobContext, JobConf job,
				TaskUmbilicalProtocol umbilical, TaskReporter reporter)
	...
	private <INKEY, INVALUE, OUTKEY, OUTVALUE> void runNewMapper(
		...
		// set the boolean flag for MapOutputWrapper to decide whether it needs wrapping
		Counters.Counter combineInputCounter = reporter
		.getCounter(COMBINE_INPUT_RECORDS);
		CombinerRunner<?, ?> combinerRunner = CombinerRunner.create(
				job, getTaskID(), combineInputCounter, reporter, null);
		combineOrNot = (combinerRunner != null) ? true : false ;
		...
		try {
			...
			if (job.getNumReduceTasks() == 0) {
				...
				output = new MapOutputWrapper(taskContext, job, umbilical,
						reporter, split, input);
			}
	...
	class MapOutputBuffer<K extends Object, V extends Object> implements
		...
		public MapOutputBuffer(TaskUmbilicalProtocol umbilical, JobConf job,
			...
			valClass = (combineOrNot) ? (Class<V>) job.getMapOutputValueClass() : 
				(Class<V>) MapOutputValueWrapper.class;
			...
			if (combinerRunner != null) {
				...
				// combineCollector = new CombineOutputCollectorWrapper<K, V>(
				// combineOutputCounter, conf, inputURI);
				combineCollector = new CombineOutputCollectorWrapper<K, V>(
						combineOutputCounter, conf, syncOffsets);
			} else {
		...
		public synchronized void collect(K key, V value, int partition)
			...
			try {
				...
				do {
					...
					final boolean kvsoftlimit = ((kvnext > kvend) ? kvnext
							- kvend > softRecordLimit
							: kvend - kvnext <= kvoffsets.length
									- softRecordLimit);
					if (kvstart == kvend && kvsoftlimit) {
						LOG.info("Spilling map output: record full = "
								+ kvsoftlimit);
						startSpill();
					}
					if (kvfull) {
...
//								codec, spilledRecordsCounter);
						...
						Class<V> myValClass = (Class<V>) MapOutputValueWrapper.class;
						writer = new Writer<K, V>(job, out, keyClass, myValClass,
								codec, spilledRecordsCounter);
				...
				if (totalIndexCacheMemory >= INDEX_CACHE_MEMORY_LIMIT) {
					...
					Path indexFilename = mapOutputFile
							.getSpillIndexFileForWrite(getTaskID(), numSpills,
			...
			}
			{
				...
				for (int parts = 0; parts < partitions; parts++) {
					...
					RawKeyValueIterator kvIter = Merger.merge(job, rfs,
							...
							job.getOutputKeyComparator(), reporter, null,
	...
	/*
	 * referred by class MapOutputWrapper and MapOutputBuffer for provenance
	 * purpose
	 */
	// protected String inputURI;
	// <key.toString(), <file \t offset, ..., file \t offset>>
	protected java.util.concurrent.ConcurrentMap<String, java.util.HashSet<String>> 
		syncOffsets = new java.util.concurrent.ConcurrentHashMap<String, java.util.HashSet<String>>(); // use
	private boolean combineOrNot = false;																															
																																																																															
	/* wrapper for provenance collection */
	private class MapOutputWrapper<K, V> extends
			org.apache.hadoop.mapreduce.RecordWriter<K, V> {
		private MapOutputCollector<K, V> collector;
		private MapOutputCollector<K, MapOutputValueWrapper> collectorWrapper;

		private final org.apache.hadoop.mapreduce.Partitioner<K, V> partitioner;
		private final int partitions;
		private ProvenanceInforGenerator provenanceGenerator;
//		private org.apache.hadoop.mapreduce.RecordReader reader;
//		private Path filePath;

		private final Log LOG = LogFactory.getLog(MapOutputWrapper.class
				.getName());

		@SuppressWarnings("unchecked")
		MapOutputWrapper(org.apache.hadoop.mapreduce.JobContext jobContext,
				JobConf job, TaskUmbilicalProtocol umbilical,
				TaskReporter reporter,
				org.apache.hadoop.mapreduce.InputSplit split,
				org.apache.hadoop.mapreduce.RecordReader input)
				throws IOException, ClassNotFoundException {
			LOG.info(((FileSplit) split).getPath().getParent().getName());				 
			
//			filePath = ((FileSplit) split).getPath();
			// fileName = ((FileSplit)split).getPath().getName();
			// String ipAddr = NetworkAddress.getIP("wlan0");
//			String ipAddr = "";
			// inputURI =
			// ProvenanceController.ProvenanceFormatter.getFileID(ipAddr,
			// fileName);
//			reader = input;
			provenanceGenerator = new ProvenanceInforGenerator(split, input);

			if (combineOrNot) {
				collector = new MapOutputBuffer<K, V> (umbilical, job, reporter);
				
				LOG.info("!!!!! we have a combiner !!!!!");
			}
			else {
				collectorWrapper = new MapOutputBuffer<K, MapOutputValueWrapper>(
						umbilical, job, reporter);
				
				LOG.info("!!!!! we do not have a combiner !!!!!");
			}
		...
		public void write(K key, V value) throws IOException,
				InterruptedException {
			// inputURI = file \t offset
			// String fileID =
			// ProvenanceController.ProvenanceFormatter.getFileID(
			// fileName, String.valueOf(reader.getCurrentOffset()));
//			String fileID = ProvenanceController.ProvenanceFormatter.getFileID(
//					filePath, String.valueOf(reader.getCurrentOffset()));
			
			String fileID = provenanceGenerator.getProvenanceInfo();

			java.util.HashSet<String> fileidSet = new java.util.HashSet<String>();
			fileidSet.add(fileID);
			java.util.HashSet<String> retlist = syncOffsets.putIfAbsent(
					key.toString(), fileidSet);
			if (retlist != null)
				retlist.add(fileID);
			...
			if (combineOrNot) {
				...
				MapOutputValueWrapper valueWrapper = new MapOutputValueWrapper();
				valueWrapper.set((Writable) value, fileID);
				valueWrapper.setConf(conf);
				collectorWrapper.collect(key, valueWrapper,
						partitioner.getPartition(key, value, partitions));
			}
		}
