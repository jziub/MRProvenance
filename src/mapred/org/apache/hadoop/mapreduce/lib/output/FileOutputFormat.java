...
public abstract class FileOutputFormat<K, V> extends OutputFormat<K, V> {
	...
	public static boolean getCompressOutput(JobContext job) {
		return job.getConfiguration().getBoolean("mapred.output.compress",
				false);
	}
	...
	public static Class<? extends CompressionCodec> getOutputCompressorClass(
			JobContext job, Class<? extends CompressionCodec> defaultValue) {
	 ...
	 * <tt>${mapred.output.dir}/_temporary/_${taskid}</tt> sub-directory for
	 * each task-attempt on HDFS where the output of the task-attempt goes. On
	 * successful completion of the task-attempt the files in the
	 * <tt>${mapred.output.dir}/_temporary/_${taskid}</tt> (only) are
	 ...
	 *         map-reduce job.
	 */
	public static Path getWorkOutputPath(
			TaskInputOutputContext<?, ?, ?, ?> context) throws IOException,
			InterruptedException {
		FileOutputCommitter committer = (FileOutputCommitter) context
				.getOutputCommitter();
	...
	public static Path getPathForWorkFile(
			TaskInputOutputContext<?, ?, ?, ?> context, String name,
			String extension) throws IOException, InterruptedException {
		return new Path(getWorkOutputPath(context), getUniqueFile(context,
				name, extension));
	}
...
//				extension));
		...
		// change to adapt to provenance collection
		Path path = new Path(committer.getWorkPath(), getUniqueFile(context, "part",
				extension));
	...
	/* for provenance collecting purpose */
