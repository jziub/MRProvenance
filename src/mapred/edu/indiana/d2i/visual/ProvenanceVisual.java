 ...
 */
package edu.indiana.d2i.visual;
...
import edu.indiana.d2i.query.MapProvenance;
import edu.indiana.d2i.query.ReduceProvenance;
import edu.indiana.d2i.query.SampleProvenanceQueryImpl;
 ...
 */
public class ProvenanceVisual extends Display {
    ...
    private final Double DEFAULT_NODE_SIZE = 5.0;
    
    public ProvenanceVisual() {
...
//        layout.add(new FruchtermanReingoldLayout(GRAPH));
    ...
    private void initDataGroups() throws IOException {
		...
		List<ReduceProvenance> allReduceProvenances = query.getAllReduceProvenances();
		// <map_task_string, node_id>
		Map<String, Integer> mapTasks = new HashMap<String, Integer>();
		// <node_id, node_size_scale>  
		Map<Integer, Double> nodeSize = new HashMap<Integer, Double>();
		// <reduce_task_id, node_id>
		Map<Integer, Integer> reduceTasks = new HashMap<Integer, Integer>(); 
		int mapSizeSum = 0;
		int reduceSizeSum = 0;
		Graph g = new Graph();
		for (MapProvenance mapProvenance : allMapProvenances) {
			// <reduce_task_id, data_size>
			TreeMap<Integer, Integer> dataDistribution = mapProvenance.getDataDistribution();
			int maptask = g.addNodeRow();
			mapTasks.put(mapProvenance.getIdentifier(), maptask);
			Iterator<Integer> iterator = dataDistribution.navigableKeySet().iterator();
			while (iterator.hasNext()) {
				Integer key = iterator.next();
				mapSizeSum += dataDistribution.get(key);
				nodeSize.put(maptask, (double)dataDistribution.get(key));
				if (reduceTasks.containsKey(key)) {
					g.addEdge(maptask, reduceTasks.get(key));
					System.out.println("edge " + maptask + " --> " + reduceTasks.get(key));
				}
				else {
					int reducetask = g.addNodeRow();
					reduceTasks.put(key, reducetask);
					g.addEdge(maptask, reduceTasks.get(key));
					System.out.println("edge " + maptask + " --> " + reduceTasks.get(key));
				}
			}
		...
		// caculate node size scale
		Iterator<Integer> iter = mapTasks.values().iterator();
		while (iter.hasNext()) {
			Integer nodeID = iter.next();
			nodeSize.put(nodeID, (double)nodeSize.get(nodeID)/mapSizeSum*DEFAULT_NODE_SIZE);
		}
        ...
        AggregateItem aitemMap = (AggregateItem)at.addItem();
        AggregateItem aitemReduce = (AggregateItem)at.addItem();
        aitemMap.setInt("id", 0);
        aitemReduce.setInt("id", 1);
        while (nodes.hasNext()) {
        	VisualItem next = (VisualItem)nodes.next();
        	if (mapTasks.values().contains(next.getRow())) {
        		next.setSize(nodeSize.get(next.getRow()));
        		aitemMap.addItem(next);
        	}
        	else {
        		next.setSize(0.5);
        		aitemReduce.addItem(next);
        	}
        }        
    }
    ...
    public static JFrame demo() {
        ProvenanceVisual ad = new ProvenanceVisual();
